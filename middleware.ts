...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (edgeRedirectsHasRedirectKey) {
      const redirectApi = new URL(`/api/redirects/${redirectKey}`, request.nextUrl.origin);
      const redirectData = await fetch(redirectApi);

      const redirectEntry: RedirectsEntry | undefined = await redirectData?.json();
      const statusCode = redirectEntry?.permanent ? 308 : 307;

      return NextResponse.redirect(new URL(redirectEntry?.destination as string, request.nextUrl.origin), statusCode);
    };
